/*! \mainpage SNOT - a Simple NOT - NOT ?! :-)
 *
 * my own small KeRNeL adapted for Arduino - now known as SNOT
 *
 * (C) 2012,2013,2014
 *
 ...
 * SNOT is a preemptive realtime kernel here ported for Arduino
 *
 * See snot.h for documentation or follow the links below
*
* NEWS : get it from github Q see https://github.com/jdn-aau/snot
* \tableofcontents
...
Use freeRam for testing (part of snot)
...
Snot itself uses approx 190B before allocating task/sem/msgQ descriptors

\section a3 Initialization
If any call fails (like no more RAM or bad parameters) SNOT will not start but will return an error code in k_start
